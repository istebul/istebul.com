...
function readPartnerApplicationFilters() {
  return {
    category: document.getElementById('partner-applications-filter-category')?.value || '',
    status: document.getElementById('partner-applications-filter-status')?.value || '',
    search: document.getElementById('partner-applications-search')?.value?.trim().toLowerCase() || ''
  };
}

function filterPartnerApplicationRows(rows = []) {
  const { category, status, search } = readPartnerApplicationFilters();
  return rows.filter((row) => {
    if (category && row.category !== category) return false;
    const rowStatus = normalizePartnerCrmStatus(row.status);
    if (status && rowStatus !== status && row.status !== status) return false;
...
function partnerFilterCategoryOptionsHtml() {
  return [
    '<option value="">Tüm kategoriler</option>',
    ...PARTNER_APP_CATEGORIES.map(
      ({ value, label }) => `<option value="${value}">${label}</option>`
    )
  ].join('');
}

function partnerFilterStatusOptionsHtml() {
  return [
    '<option value="">Tüm durumlar</option>',
    ...Object.entries(PARTNER_APP_STATUS_LABELS).map(
      ([value, label]) => `<option value="${value}">${label}</option>`
    )
  ].join('');
}
...
export async function loadPartnerApplications(ctx) {
  ...
  const { adminAction, escapeHtml } = ctx;
  ...
  renderPartnerApplicationsView(ctx);
...
function renderPartnerApplicationsView(ctx) {
  const el = document.getElementById('partner-applications-list');
  if (!el) return;

  const { escapeHtml } = ctx;
  const filteredRows = filterPartnerApplicationRows(cachedRows);
  ...
  const summary = `<p class="text-muted-sm" style="margin:0 0 12px">${filteredRows.length} / ${cachedRows.length} kayıt gösteriliyor</p>`;

  if (!filteredRows.length) {
    el.innerHTML = `
      ${pipelineBoard}
      ${summary}
      <p class="empty">Filtreye uygun başvuru yok.</p>`;
    return;
  }
  ...
  el.innerHTML = `
    ${pipelineBoard}
    ${summary}
    ${renderPartnerApplicationsTable(ctx, filteredRows)}
  `;
}
...
export function bindPartnerApplicationsAdminUi(ctx) {
  ...
  document.getElementById('partner-applications-filter-category')?.addEventListener('change', () => {
    renderPartnerApplicationsView(ctx);
  });

  document.getElementById('partner-applications-filter-status')?.addEventListener('change', () => {
    renderPartnerApplicationsView(ctx);
  });

  document.getElementById('partner-applications-search')?.addEventListener('input', () => {
    renderPartnerApplicationsView(ctx);
  });

  document.getElementById('partner-applications-clear-filters')?.addEventListener('click', () => {
    const category = document.getElementById('partner-applications-filter-category');
    const status = document.getElementById('partner-applications-filter-status');
    const search = document.getElementById('partner-applications-search');
...
export function getPartnerApplicationFormMarkup() {
  return `
    <div class="partner-applications-toolbar" style="display:flex;gap:10px;flex-wrap:wrap;align-items:center;margin-bottom:16px">
      ...
      <select id="partner-applications-filter-category" class="status-select" aria-label="Kategori filtresi">${partnerFilterCategoryOptionsHtml()}</select>
      <select id="partner-applications-filter-status" class="status-select" aria-label="Durum filtresi">${partnerFilterStatusOptionsHtml()}</select>
      <input id="partner-applications-search" type="search" placeholder="Ara: firma, e-posta, telefon…" style="min-width:220px">
      <button type="button" class="btn btn-ghost btn-sm" id="partner-applications-clear-filters">Filtreleri temizle</button>
    </div>
