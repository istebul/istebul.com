/**
 * Karar Nabzı — feature flag (opt-in, default off).
 */

export const KARAR_NABZI_URL_PARAM = 'karar_nabzi';
export const KARAR_NABZI_STORAGE_KEY = 'istebul_karar_nabzi_flag';
...
function readUrlFlag(searchParamsLike) {
  try {
    ...
    return parseTruthyFlag(params.get(KARAR_NABZI_URL_PARAM));
  } catch {
    return null;
  }
}
...
function readStorageFlag(storageLike) {
  try {
    ...
    return parseTruthyFlag(storage.getItem(KARAR_NABZI_STORAGE_KEY));
  } catch {
    return null;
  }
}
 ...
 * @returns {boolean}
 */
export function isKararNabziEnabled(searchParamsLike, storageLike) {
