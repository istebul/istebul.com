-- Vertical partner dispatch P0: canonical route types, lead partner fields, kasko_leads

-- Extend partner_endpoints.route_type for vertical funnels
...
-- Partner dispatch fields on vertical lead tables
...
create index if not exists kasko_leads_dispatch_retry_idx
  on public.kasko_leads (partner_dispatch_status, partner_dispatch_next_retry_at)
  where partner_dispatch_status = 'failed';
...
alter table public.housing_leads
  drop constraint if exists housing_leads_partner_dispatch_status_check;
alter table public.housing_leads
  add constraint housing_leads_partner_dispatch_status_check
    check (partner_dispatch_status in ('pending', 'sent', 'failed', 'dead'));

alter table public.vacation_leads
  drop constraint if exists vacation_leads_partner_dispatch_status_check;
alter table public.vacation_leads
  add constraint vacation_leads_partner_dispatch_status_check
    check (partner_dispatch_status in ('pending', 'sent', 'failed', 'dead'));

alter table public.sigorta_leads
  drop constraint if exists sigorta_leads_partner_dispatch_status_check;
alter table public.sigorta_leads
  add constraint sigorta_leads_partner_dispatch_status_check
    check (partner_dispatch_status in ('pending', 'sent', 'failed', 'dead'));

alter table public.vertical_leads
  drop constraint if exists vertical_leads_partner_dispatch_status_check;
alter table public.vertical_leads
  add constraint vertical_leads_partner_dispatch_status_check
    check (partner_dispatch_status in ('pending', 'sent', 'failed', 'dead'));

alter table public.kasko_leads
  drop constraint if exists kasko_leads_partner_dispatch_status_check;
alter table public.kasko_leads
  add constraint kasko_leads_partner_dispatch_status_check
    check (partner_dispatch_status in ('pending', 'sent', 'failed', 'dead'));

-- Retry indexes
create index if not exists housing_leads_dispatch_retry_idx
  on public.housing_leads (partner_dispatch_status, partner_dispatch_next_retry_at)
  where partner_dispatch_status = 'failed';

create index if not exists vacation_leads_dispatch_retry_idx
  on public.vacation_leads (partner_dispatch_status, partner_dispatch_next_retry_at)
  where partner_dispatch_status = 'failed';

create index if not exists sigorta_leads_dispatch_retry_idx
  on public.sigorta_leads (partner_dispatch_status, partner_dispatch_next_retry_at)
  where partner_dispatch_status = 'failed';

create index if not exists vertical_leads_dispatch_retry_idx
  on public.vertical_leads (partner_dispatch_status, partner_dispatch_next_retry_at)
  where partner_dispatch_status = 'failed';

-- Dispatch logs: lead source discriminator
alter table public.partner_lead_dispatch_logs
  add column if not exists lead_source text not null default 'auto_leads';

alter table public.partner_lead_dispatch_logs
  drop constraint if exists partner_dispatch_trigger_check;

alter table public.partner_lead_dispatch_logs
  add constraint partner_dispatch_trigger_check
...
create index if not exists partner_dispatch_logs_lead_source_idx
  on public.partner_lead_dispatch_logs (lead_source, lead_id, created_at desc);
