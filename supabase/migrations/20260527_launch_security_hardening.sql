-- Launch security hardening (idempotent).
-- Same policies also appear in 20260527_auto_lead_ai_summary.sql; safe to re-apply.

-- Admin CRM: allow SELECT on operational tables (writes via admin-action / service role)
...
-- Funnel aggregate view: not exposed to anon/authenticated clients
DO $$
BEGIN
  IF EXISTS (
