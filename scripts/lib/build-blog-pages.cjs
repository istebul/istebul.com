...
async function fetchPostsFromSupabase(env = process.env) {
  ...
  const query = `posts?select=${POST_SELECT}&is_published=eq.true&content_type=eq.blog&order=is_featured.desc,created_at.desc&limit=24`;
  try {
...
function postToSeoPage(post) {
  const intro = post.excerpt || post.body.slice(0, 240);
  const bodyParagraph =
    post.body && post.body.length > intro.length
      ? post.body
      : `${intro} Bu yazı bilgilendirme amaçlıdır; yatırım veya kredi tavsiyesi değildir. Kişisel kararınız için /auto/ üzerinden ücretsiz analiz başlatabilirsiniz.`;
