...
export function normalizeWriteRequest(body, requestId) {
  ...
  if (action === "execute_item") {
    const putawayId = normalizeUuid(payload.putawayId);
    if (!putawayId) {
      return { ok: false, reason: "putaway_id_invalid" };
    }

    const putawayItemId = normalizeUuid(payload.putawayItemId);
    if (!putawayItemId) {
      return { ok: false, reason: "putaway_item_id_invalid" };
    }

    const targetLocationId = normalizeUuid(
...
async function invokePutawayWrite(
  env,
  token,
  input,
  fetchImpl,
) {
  const isExecute = input.action === "execute_item";
  ...
  const rpcBody = isComplete
    ? {
        ...
        p_putaway_id: input.payload.putawayId,
      }
    : isExecute
      ? {
          p_request_id: input.requestId,
          p_account_id: input.accountId,
          p_putaway_id: input.payload.putawayId,
          p_putaway_item_id: input.payload.putawayItemId,
          p_target_location_id: input.payload.targetLocationId,
          p_quantity: input.payload.quantity,
          p_notes: input.payload.notes ?? null,
        }
      : {
  ...
  try {
    ...
    if (!normalized.ok) {
      const messages = {
        ...
        action_invalid:
          "Yerleştirme işlemi desteklenmiyor.",
        request_id_invalid:
          "Idempotency-Key başlığı geçerli bir UUID olmalıdır.",
        payload_invalid:
          "Yerleştirme işlem verisi JSON nesnesi olmalıdır.",
        putaway_id_invalid:
          "Yerleştirme kimliği geçerli bir UUID olmalıdır.",
        putaway_item_id_invalid:
          "Yerleştirme satırı kimliği geçerli bir UUID olmalıdır.",
        target_location_id_invalid:
          "Hedef lokasyon kimliği geçerli bir UUID olmalıdır.",
        quantity_invalid:
          "Yerleştirme miktarı sıfırdan büyük olmalıdır.",
        notes_invalid:
          "Yerleştirme notu metin olmalıdır.",
      };
      ...
      return corsJsonError(
        ...
        messages[normalized.reason] ||
          "Yerleştirme isteği geçersizdir.",
        origin,
        undefined,
        responseHeaders(),
      );
    ...
    const result = await invokePutawayWrite(
      context.env,
      token,
      normalized.value,
      fetchImpl,
    ...
    if (!result.ok) {
      ...
      logApiEvent(
        ...
        "warehouse_putaway_write_failed",
        {
          userId: auth.user.id,
    ...
    logApiEvent("info", "warehouse_putaway_write_succeeded", {
      userId: auth.user.id,
      accountId: normalized.value.accountId,
      action: normalized.value.action,
      requestId: normalized.value.requestId,
    ...
    logApiEvent(
      "error",
      "warehouse_putaway_write_unhandled",
      {
        message:
    ...
    return corsJsonError(
      ...
      "Yerleştirme işlemi sırasında beklenmeyen bir hata oluştu.",
      origin,
      undefined,
      responseHeaders(),
    );
